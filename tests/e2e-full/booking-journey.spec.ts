...
test.describe("Appointment Booking Journey", () => {
  ...
  test.describe("Complete Booking Flow", () => {
    test("completes full booking journey from homepage to confirmation", async ({
      ...
      await expect(
        page.getByRole("heading", { name: /select.*date/i })
      ).toBeVisible();
      ...
      await expect(
        page.getByRole("heading", { name: /complete.*booking/i })
      ).toBeVisible();
      ...
      await expect(
        page.getByText(/appointment.*booked.*successfully/i)
      ).toBeVisible();
    ...
    test("shows appropriate messages for no available slots", async ({
      page,
    }) => {
    ...
    test("maintains booking progress state", async ({ page }) => {
      ...
      const serviceTitle = await firstService
        .getByRole("heading")
        .textContent();
  ...
  test.describe("Accessibility", () => {
    test("supports keyboard navigation throughout booking flow", async ({
      page,
    }) => {
      await page.goto("/book");
    ...
    test("provides proper ARIA labels and roles", async ({ page }) => {
      ...
      const serviceButton = page
        ...
        .first();
      await expect(serviceButton).toHaveAttribute(
        "aria-label",
        /select.*service/i
      );
