...
test.describe("Contact System E2E", () => {
  ...
  test.describe("Contact Form", () => {
    ...
    test("shows validation errors for empty required fields", async ({
      ...
      await expect(
        page.getByText("Subject must be at least 5 characters")
      ).toBeVisible();
      await expect(
    ...
    test("submits form successfully with valid data", async ({ page }) => {
      ...
      await page
        ...
        .fill("Interested in counseling services");
      await page
        .getByLabel("Message", { exact: true })
        .fill(
          "I would like to schedule an appointment for individual therapy sessions."
        );
    ...
    test("displays crisis support resources", async ({ page }) => {
      ...
      await expect(
        page.getByRole("heading", { name: "Crisis Support Resources" })
      ).toBeVisible();
      await expect(
        page.getByText("National Suicide Prevention Lifeline:")
      ).toBeVisible();
    ...
    test("displays contact information and office hours", async ({ page }) => {
      ...
      // Check office hours
      await expect(
        page.getByRole("heading", { name: "Office Hours" })
      ).toBeVisible();
      await expect(
        page.getByText("Monday - Thursday: 9:00 AM - 7:00 PM")
      ).toBeVisible();
    ...
    test("displays FAQ section", async ({ page }) => {
      ...
      await expect(
        page.getByText("How do I schedule my first appointment?")
      ).toBeVisible();
      await expect(page.getByText("Do you accept insurance?")).toBeVisible();
      await expect(
        page.getByText("What should I expect in my first session?")
      ).toBeVisible();
  ...
  test.describe("Admin Authentication", () => {
    test("admin login page displays correctly", async ({ page }) => {
      ...
      await expect(
        page.getByText("Email: admin@healingpathways.com")
      ).toBeVisible();
  ...
  test.describe("Admin Dashboard", () => {
    ...
    test("displays contact submissions dashboard", async ({ page }) => {
      ...
      await expect(
        page.getByText("Manage and respond to contact form submissions")
      ).toBeVisible();
    ...
    test("can view submission details and send response", async ({ page }) => {
      ...
      // Check response form
      await expect(
        page.getByRole("heading", { name: "Send Response" })
      ).toBeVisible();
      ...
      await page
        .getByLabel("Message")
        .fill(
          "Thank you for reaching out. I would be happy to schedule an appointment with you. Please call our office at (555) 123-4567 to set up a time."
        );
  ...
  test.describe("Navigation Integration", () => {
    test("can navigate to contact page from main navigation", async ({
      page,
    }) => {
      await page.goto("/");
