...
test.describe('Registration User Journey', () => {
  ...
  test('complete registration flow from start to verification page', async ({ page }) => {
    ...
    await expect(page.getByText('Email address is available')).toBeVisible({ timeout: 10000 });
    ...
    await expect(page).toHaveURL(/\/auth\/verify-email\?email=john\.doe%40example\.com/);
    
    // Verify verification page content
    await expect(page.getByText('Check Your Email')).toBeVisible();
    await expect(page.getByText('Verification Email Sent')).toBeVisible();
    await expect(page.getByTestId('email-display')).toContainText('john.doe@example.com');
    ...
    await expect(page.getByTestId('resend-message')).toBeVisible({ timeout: 10000 });
  ...
  test('handles email already exists scenario', async ({ page }) => {
    ...
    await expect(page.getByText('Email address is already registered')).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.getByText(/email address is already registered/i)).toBeVisible();
  });
  ...
  test('password requirements and strength indicator work correctly', async ({ page }) => {
  ...
  test('navigation and page structure are correct', async ({ page }) => {
    ...
    // Check hero section
    await expect(page.getByText('Create Your Account')).toBeVisible();
    await expect(page.getByText('Join our community and take the first step')).toBeVisible();
  ...
  test('handles network errors gracefully', async ({ page }) => {
    ...
    await expect(page.getByText('Error checking email availability')).toBeVisible({ timeout: 10000 });
    ...
    await expect(page).toHaveURL(/\/auth\/verify-email\?email=test%40example\.com/);
  });
  ...
  test('handles registration API errors', async ({ page }) => {
    ...
    await page.route('/api/auth/check-email*', route =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ available: true, message: 'Email address is available' })
      })
    );
    ...
    await page.route('/api/auth/register', route =>
      route.fulfill({
        status: 400,
        contentType: 'application/json',
        body: JSON.stringify({ error: 'Registration failed', details: 'Server error' })
      })
    );
...
test.describe('Verify Email Page Direct Access', () => {
  test('displays correctly when accessed with email parameter', async ({ page }) => {
  ...
  test('resend verification functionality works', async ({ page }) => {
    ...
    await expect(page.getByTestId('resend-message')).toBeVisible({ timeout: 3000 });
    await expect(page.getByText('Verification email sent! Please check your inbox.')).toBeVisible();
  });
  ...
  test('support section provides helpful information', async ({ page }) => {
    ...
    await expect(page.getByText('What Happens After Verification?')).toBeVisible();
    await expect(page.getByText('Your account will be fully activated')).toBeVisible();
  });
  ...
  test('form fields have proper autocomplete attributes', async ({ page }) => {
    ...
    await expect(page.getByTestId('phone-input')).toHaveAttribute('autocomplete', 'tel');
  ...
  test('real-time email validation shows visual feedback', async ({ page }) => {
    ...
    await expect(page.getByText('Email address is available')).toBeVisible({ timeout: 3000 });
