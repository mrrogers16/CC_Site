...
import {
  POST as AdminResponse,
  PATCH as AdminUpdate,
} from "@/app/api/admin/contact/[id]/route";
...
describe("Contact System Integration", () => {
  ...
  describe("Admin Response Flow", () => {
    ...
    it("marks submission as read and sends response", async () => {
      ...
      // Mock database operations
      (prisma.contactSubmission.findUnique as jest.Mock).mockResolvedValue(
        mockSubmission
      );
      ...
      const request = new NextRequest(
        `http://localhost:3000/api/admin/contact/${submissionId}`,
        {
          method: "POST",
      ...
      const response = await AdminResponse(request, {
    ...
    it("updates submission read status", async () => {
      ...
      const response = await AdminUpdate(request, {
        params: Promise.resolve({ id: submissionId }),
      });
      const result = await response.json();
    ...
    it("requires authentication for admin operations", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/admin/contact/test-id",
        {
          method: "PATCH",
          body: JSON.stringify({ isRead: true }),
          headers: { "Content-Type": "application/json" },
